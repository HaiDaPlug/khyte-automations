  ...
  const caseCard = ({ slug, index, company, description, gradient, metrics }: Pick<CaseData, "slug" | "index" | "company" | "problem" | "description" | "gradient" | "metrics">) => (
    ...
    >
      ...
      <div className="flex flex-col flex-1 p-6 pt-5">
        ...
        {/* Key metric
             One size for every card. The value slot holds two kinds of content:
             a short figure ("≈32h") and a longer phrase ("Bokning → schema →
             bekräftelse") for cases whose result is not a single number. Sizing
             each to its own content made the grid look inconsistent — cards
             scanned side by side should share one voice — so this sits between
             the two: enough weight to read as a result, small enough that a
             phrase wraps cleanly without competing with the card title. */}
        <div className="mt-6 pt-5 border-t border-[rgba(58,51,48,0.12)]">
          <p
            className="font-jakarta text-[var(--color-text)] mb-1.5"
            style={{ fontSize: "clamp(1.15rem, 1.6vw, 1.35rem)", fontWeight: 800, lineHeight: 1.25, letterSpacing: "-0.01em", textWrap: "balance" }}
          >
            {metrics[0].value}
            {metrics[0].unit && (
              <span
