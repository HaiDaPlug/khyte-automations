...
export default function KiteHero() {
  ...
  const CENTER_Y = (TIP.y + BELLY.y) / 2;
  ...
  return (
    <div
      ...
      style={{ width: "100%", maxWidth: 400, aspectRatio: "3 / 4" }}
      aria-hidden="true"
    >
      <svg
        viewBox="0 0 400 560"
      ...
      >
        ...
        {tailSegments.map((seg, i) => (
          <motion.rect
            key={i}
            x={seg.baseX} y={seg.baseY}
            width={seg.w} height={seg.h} rx={seg.rx}
        ...
        {emberParticles.map((p, i) => (
          <motion.circle
            key={i}
            cx={0} cy={0} r={p.r} fill={p.fill}
            style={{ x: p.x, y: p.y, opacity: p.o }}
          />
        ))}

        <motion.g style={{ y: kiteY, rotate: kiteRot, originX: `${TIP.x}px`, originY: `${CENTER_Y}px` }}>
          <path
            d={`M ${TIP.x+14} ${TIP.y+22} L ${LEFT.x+14} ${LEFT.y+22} L ${BELLY.x+14} ${BELLY.y+22} L ${RIGHT.x+14} ${RIGHT.y+22} Z`}
            fill="rgba(0,0,0,0.30)" opacity={0.6}
          />
        </motion.g>

        <motion.g style={{ y: kiteY, rotate: kiteRot, originX: `${TIP.x}px`, originY: `${CENTER_Y}px` }}>
          <path d={`M ${TIP.x} ${TIP.y} L ${LEFT.x} ${LEFT.y} L ${TIP.x} ${LEFT.y} Z`} fill="url(#kFA)" />
          <path d={`M ${TIP.x} ${TIP.y} L ${RIGHT.x} ${RIGHT.y} L ${TIP.x} ${RIGHT.y} Z`} fill="url(#kFB)" />
          <path d={`M ${LEFT.x} ${LEFT.y} L ${BELLY.x} ${BELLY.y} L ${TIP.x} ${LEFT.y} Z`} fill="url(#kFC)" />
          <path d={`M ${RIGHT.x} ${RIGHT.y} L ${BELLY.x} ${BELLY.y} L ${TIP.x} ${RIGHT.y} Z`} fill="url(#kFD)" />
          <path
          ...
          />
          <line x1={TIP.x} y1={TIP.y} x2={BELLY.x} y2={BELLY.y} stroke="url(#kSP)" strokeWidth="1.8" />
          <line x1={LEFT.x} y1={LEFT.y} x2={RIGHT.x} y2={RIGHT.y} stroke="rgba(255,255,255,0.18)" strokeWidth="1.4" />
