# Khyte Automations — SEO Audit & Growth Playbook
**Date:** 2026-03-05
**Domain:** khyte.se
**Status:** Live site, early-stage (limited content, 1 testimonial)
**Last reconciled against the codebase:** 2026-08-29 — see "Reconciliation" below.


> **⚠️ Reconciliation — 2026-08-29.** This audit was written 2026-03-05 and parts of it are now
> stale. Corrected since it was written:
>
> - **Domain is `khyte.se`**, not `khyteautomations.com`. All URLs in this document have been rewritten.
> - **Public contact email is `hai@khyte.se`**, not `hai@khyteteam.com`.
> - **GA4 is installed** (`G-F91HE9L5LS`, lazy-loaded in `layout.tsx`) and Vercel Analytics is live.
>   The "no analytics" risk is closed.
> - **URLs are already Swedish** (`/tjanster`, `/case`, `/om-oss`, `/kontakt`) with permanent 301s
>   from the English paths in `next.config.ts`. The Medium-priority migration item is done.
> - **Sitemap covers all 7 static routes plus every case detail page**, generated from `src/data/cases.ts`.
>   The "missing /services" risk is closed.
> - **LocalBusiness (ProfessionalService) and FAQPage schema are live.** FAQ schema is now emitted
>   per-page from `src/data/faq.ts` so it always matches the visible accordion.
> - **OG image is a real 1200×630 PNG** generated by `src/app/opengraph-image.tsx`. The SVG is gone.
> - **`/internal/` is disallowed** in `robots.ts` and password-gated in `src/proxy.ts`.
>
> Still open: Google Business Profile, GSC verification, Service + BreadcrumbList schema,
> `/boras` landing page, the blog, tool-integration pages, and every backlink/outreach item.
